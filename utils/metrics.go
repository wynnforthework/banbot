...
func calcDrawDowns(profits []float64, initBalance float64) ([]float64, []float64, []float64, []float64) {
	cumulative := make([]float64, len(profits))  // 累计收益
	highs := make([]float64, len(profits))       // 累计收益最高值
	drawdown := make([]float64, len(profits))    // 每个周期累计最大亏损（负数）
	drawdownPct := make([]float64, len(profits)) // 每个周期回撤百分比
